...
def build_new_chat_runtime_context(
    *,
    ...
    """``mentioned_document_ids`` and ``mentioned_folder_ids`` are consumed by
    the ``search_knowledge_base`` tool, which turns them into its
    ``SearchScope``: the pinned documents plus the pinned folders' subtrees.
